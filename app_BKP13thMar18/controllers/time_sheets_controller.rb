class TimeSheetsController < ApplicationController
  ...
  def import
    #raise params[:file1].original_filename.inspect
     if (params[:file1])
        filename = params[:file1].original_filename.split('_')
                 @Arrfile = TimeSheet.where('("WORKED_ID")= ?',filename[2])
                   #raise @Arrfile.inspect
                 if !@Arrfile.blank?
                       flash[:notice] = 'TIME SHEET ALREADY TAKEN'
